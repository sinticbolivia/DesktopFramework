...
namespace SinticBolivia
{
	public class SBDateTime : Object
	{
		...
		public bool parse(string dateStr, string dateSep = "-")
		{
			...
			{
				var partsDateTime = (dateStr.index_of ("T") > -1) ? dateStr.strip().split("T") : dateStr.strip().split (" ");
				var partDate = partsDateTime[0].strip();
				var partTime = partsDateTime[1].strip();
				//-- This is a time
				var partsTime = partTime.strip().split (":");
				//var part = parts[0];
				var partsDate = partDate.strip().split(dateSep); //part.index_of ("T") > -1 ? part.split("T") : part.split (" ");
				...
				var timeParts = r.split (partTime.strip());
				//print("DATE PARTS: %s\n", string.joinv(",", partsDate)/*parseDateStr*/ );
				...
				for (var i = 1; i < timeParts.length - 1; i++)
				{
					...
					{
						if( timeParts[i].index_of(".") != -1 )
						{
							second = timeParts[i].split(".")[0];
							miliSecs = timeParts[i].split(".")[1];
						}
						else
							second = timeParts[i];
					}
				}
				//print("TIME: %s:%s:%s.%s\n", hour, minute, second, miliSecs);
				this.parse_date (partDate);
				if( this._valid )
				{
					//print("IS VALID\n");
					/*
